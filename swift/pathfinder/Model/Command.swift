...
//MARK: Was struct but probably should be class, change back to struct if problems
class Command {
...
func generateCommands(initialState: Command, path: [Node]) -> [MQTTCommand] {
    ...
    for i in 0..<(path.count - 1) {
        ...
        /*
         ...
         - Keep track of current rotation / heading
         - If there is no rotation change just sum up all nodes following the same heading to get distance (NumberOfNodes * 9.75)
         - If heading changes then calculate rotation needed and do the above
         - Also if moving diagonal, distance * sqrt(2) = true distance (since moving diagonally is longer)
         
         - Send distnace / 0.15 to SMCE to convert to centimeters
